...
# Legal term definitions in simple language
_DEFINITIONS = {
...
def extract_legal_terms(text: str):
    ...
    for match in re.finditer(quoted_pattern, text):
        ...
        if term_lower not in seen:
            ...
            definition = _DEFINITIONS.get(term_lower, None)
            found.append({
                "term": term,
    ...
    for keyword, category in _LEXICON.items():
        if keyword in t:
            if keyword not in seen:
                definition = _DEFINITIONS.get(keyword, None)
                found.append({
                    "term": keyword.title(),
                    "category": category,
                    "definition": definition
        ...
        try:
            ...
            for ent in doc.ents:
                if ent.label_ in ['LAW', 'ORG', 'EVENT']:
                    # Clean up entity text - remove quotes, double words, etc.
                    ...
                    # Skip if has repeated words like "the the"
                    ...
                    if ent_text_lower not in seen:
                        # Get definition if available
                        definition = _DEFINITIONS.get(ent_text_lower, None)
                        found.append({
                            "term": ent_text,
